...
- **État carte** : `resolvedAleatoireCells` (Dict string→CellType — types résolus des cases Aleatoire) · `postVisitCellTypes` (Dict string→CellType — état post-visite) · `overridesMaximum` (Dict string→CellType — cases remplacées par le système de maximum) · `carteInitialisee` (bool). Tous clearés dans `StartNewRun()`.
- `GetEffectiveCellType(CellData)` : **source de vérité unique** pour le type runtime d'une case. Priorité : `postVisitCellTypes` > `overridesMaximum` > `resolvedAleatoireCells` (si cellType == Aleatoire) > `cell.cellType`. Utilisé par `NavigationManager.OnRoomEntered()`, `TryMove()`, et `MapRenderer.ObtenirTypeAffiche()`.
- `ClearCurrentRoom()` : ajoute à `clearedRooms` + appelle `DeterminerPostVisitType(currentCellType)` → `SetPostVisitType`. `currentCellType` est déjà le type effectif (setté par `EnterRoom` via `GetEffectiveCellType`).
- `DeterminerPostVisitType(CellType)` : `Ferrailleur→FerailleurUtilise`, `Teleporteur→TeleporteurUtilise`, `Shop→Shop`, tous les autres (PointInteret, Radar, Coffre, CombatSimple, Event…) → `Empty`.
...
**`NavigationManager.cs`** — Déplacements clavier, brouillard de guerre (3 sets), `AppliquerEffetsNav`, `RevealZoneChoice`, tirage d'events (`ChoisirEventAleatoire` + fallback `RandomEvents`). Assigne `currentMapData` avant `EnterRoom()` pour les cases CombatSimple/Elite/Boss et avant `GoToShop()`. Gère les cooldowns des skills de navigation : `SpawnSkillsJambes()` grise les boutons selon `IsNavSkillReady`, `UtiliserSkillJambes()` appelle `SetNavSkillCooldown` après effet. Détecte la première visite d'un shop (`GetShopState == null`) pour déclencher `TickCooldownsDe(ShopDecouvert)`.
- `InitialiserCarte()` : appelée dans `Start()` si `!RunManager.carteInitialisee`. Étape 1 = enforcement des maximums par type (Fisher-Yates sur les cases en excès → `RunManager.SetOverrideMaximum`). Étape 2 = résolution des cases `Aleatoire` via `CellAleaPool.TirerAleatoire` (Fisher-Yates sur l'ordre de tirage, comptes actuels passés en paramètre pour respecter les `maxOccurrences`).
- `VerifierBloqueurLD()` : parcourt toutes les cases `BloqueurLD`, évalue leur `BloqueurCondition` via `EvaluerConditionBloqueur`, convertit en `Empty` via `SetPostVisitType` si condition remplie. Skip si `HasPostVisitType` déjà posé. Appelée dans `Start()` après `InitialiserCarte()`, avant le placement du joueur. Warning si `condition == null` ou `compteurID` vide.
- `OnRoomEntered()` : utilise `RunManager.GetEffectiveCellType()` — le switch ne reçoit jamais le type SO brut. Cases couvertes : `CombatSimple/Elite/Boss` (→ Combat), `Event` (→ `ChoisirEventAleatoire` + GoToEvent), `Shop` (→ GoToShop), `PointInteret` (→ `cell.specificEvent` + GoToEvent, warning si null), `Radar`/`Coffre` (→ `ChoisirEventAleatoire` + GoToEvent), `Teleporteur` (→ `cell.specificEvent` + GoToEvent, warning si null), `Ferrailleur` (placeholder `[Chantier futur]` → `ChoisirEventAleatoire` + GoToEvent), `FerailleurUtilise`/`TeleporteurUtilise` (log + break), `Start`/`Empty` (log + break). Pas d'`IsRoomCleared` sur les nouveaux types — le postVisitType (Empty/TeleporteurUtilise/FerailleurUtilise) redirige automatiquement via `GetEffectiveCellType`.

**`MapData.cs`** — ScriptableObject grille. `CellData` a `specificEnemy` (EnemyData), `specificGroup` (EnemyGroup — prioritaire sur `specificEnemy`), `eventList`/`eventPool`, `shopData`, `specificEvent` (EventData — pour PointInteret et Teleporteur), `condition` (BloqueurCondition — pour BloqueurLD). `MapData` a `normalEnemyPool`, `eliteEnemyPool`, `bossEnemyPool` (EnemyPool), `defaultCombatLootTable` (EquipmentLootTable fallback si l'ennemi/groupe n'a pas de lootPool), `defaultLootOfferCount` (int, 1–4), `aleatoirePool` (CellAleaPool), `maximumsParType` (List\<MaxTypeEntry\>), `typeDeRemplacement` (CellType). Nouvelles classes : `BloqueurCondition` (`type` BloqueurConditionType + `compteurID` string + `valeurCible` int) · `MaxTypeEntry` (`type` CellType + `maximum` int). `BloqueurConditionType` enum : `CompteurNomme(0)` / `CombatsTermines(1)` / `EventsTermines(2)`.
...
| `CellAleaPool` | Pool de types de cases pour la résolution des cases `Aleatoire`. `List<CellAleaEntry> entrees` (chaque entrée : `type` CellType, `poids` int, `maxOccurrences` int). `TirerAleatoire(Dictionary<CellType,int> comptesActuels)` : tirage pondéré, exclut les entrées dont `poids <= 0` ou dont le quota `maxOccurrences` est atteint. Retourne `CellType.Empty` si aucune entrée disponible. |
...
22. **`CellType` enum** : ne jamais insérer au milieu (valeurs sérialisées en int). Ajouter **à la fin**. Ordre actuel : Empty(0) Start(1) Boss(2) CombatSimple(3) Event(4) NonNavigable(5) Shop(6) Elite(7) BloqueurLD(8) PointInteret(9) Ferrailleur(10) Radar(11) Coffre(12) Teleporteur(13) Aleatoire(14) FerailleurUtilise(15) TeleporteurUtilise(16).
