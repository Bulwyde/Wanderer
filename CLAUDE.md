...
- `InitializeCombat()` : crée `_armsController` + appelle `SpawnArmsUI()` après `ResolveEquipment()`. Applique `spawnEffects` de chaque ennemi via `ApplyEnemyEffect` avant le premier tour.
- `EndCombat()` : appelle `_armsController.Clear()` pour cleanup GameObjects orphelins.
- `UpdateSkillButtons()` : met à jour l'état des `SkillIconButton` (icones armes) — cooldown, coût effectif (après réductions stat), `SetInteractable` selon énergie/tour/ciblage.
...
### Système de Tooltips — Affichage description/effets au survol (Objectif 2)
...
**`TooltipTrigger.cs`** — Composant attaché aux éléments UI. Gère détection survol + données.
- `SetTooltipData(string name, string description, List<TagData> tags)` : configure (appelé lors du Setup du composant parent).
- `OnPointerEnter(PointerEventData)` : déclenche `TooltipController.ShowTooltip()`.
- `OnPointerExit(PointerEventData)` : appelle `TooltipController.OnHoverElementExit()` — masque le tooltip seulement si la souris n'est pas dessus (anti-flickering).
- Implémente `IPointerEnterHandler` + `IPointerExitHandler`. Champs `itemName`/`itemDescription`/`itemTags` privés, non exposés (toujours écrasés par `SetTooltipData` au runtime).
...
**Prefab `TooltipPanel`** — instancié une seule fois sous le Canvas par `ShowTooltip()`, puis réutilisé (réactivé + repeuplé). Enfants cherchés par **nom exact** via `Find()` : `NameText`, `DescriptionText`, `TagsContainer` — un nom mal orthographié = champ silencieusement ignoré. `TagsContainer` porte un `HorizontalLayoutGroup` ; `PopulateTooltip()` y instancie un badge `TagBadge` par tag, filtré sur `tag.affiché == true`.

**Intégration dans les composants**
Tous les composants UI appelent `SetTooltipData()` dans `Setup()` :
- **`SkillIconButton.cs`** (icones armes — skills de combat) : `trigger.SetTooltipData(skill.skillName, skill.description, skillTags)`.
...
1. **Déclenchement au survol** : `TooltipTrigger` ouvre au `OnPointerEnter`, ferme au `OnPointerExit` via `OnHoverElementExit()`. La coroutine `MonitorTooltipHover()` (tick 0.1s) gère les conflits raycast et la souris qui traverse le tooltip. ⚠️ Cohérence obligatoire : `TooltipTrigger` ET `TooltipController` doivent utiliser la même mécanique (survol) — un trigger clic droit + un controller survol fermait le tooltip ~0.1s après ouverture.
...
7. **`CurrentHoverElement`** : propriété publique exposant le dernier élément survolé — mise à jour dans `ShowTooltip`, vidée dans `DisappearRoutine`. Sert au suivi interne du hover ; non consommée hors `TooltipController` depuis le passage en survol pur (accesseur conservé pour usage futur).
...
| `StatusData` | Statut (behavior, perTurnAction, decayPerTurn, decayTiming, maxStacks) | `decayConditionTag` : filtre tag pour `OnSkillUse`. `passiveEffects` : `List<EffectData>` déclenchées par trigger quand stacks > 0. Custom editor masque les sections inutilisées selon behavior/decayTiming. |
...
**Editors custom :** `EffectDataEditor`, `EventEffectDrawer`, `NavEffectDrawer`, `MapEditorWindow`, `TagDataEditor`, `SkillDataEditor`, `StatusDataEditor`.
...
**Prefabs :** `ChoiceButton` (360×65px), `SkillIconButton`, `LootCard`, `ConsumableButton`, `ModuleIcon` (48×48px), `StatusIconPrefab`.
...
**StatusDecayTiming :** `OnTurnStart` (défaut), `OnTurnEnd`, `OnSkillUse`, `OnDamageTaken`, `OnArmorGain`, `OnHealing`. `DecayPlayerStatuses(timing, usedSkill = null)` — le paramètre `usedSkill` sert au filtre de tag `OnSkillUse` : si `status.decayConditionTag != null`, le skill utilisé doit porter ce tag. Signature étendue — toujours passer le skill dans les branches de ciblage et direct. **Ordre obligatoire : `ApplyStatusPassivesForPlayer` AVANT `DecayPlayerStatuses`** (passifs d'abord, décroissance ensuite — respecté dans StartPlayerTurn, OnEndTurn, UseSkill, OnEnemyCibleClique, AppliquerDegatsAuJoueur, ApplyEffect).

**Passifs de statut (`StatusData.passiveEffects`) :** `List<EffectData>` portée par chaque `StatusData`. `ApplyStatusPassivesForPlayer(EffectTrigger trigger, SkillData usedSkill = null)` itère les statuts actifs du joueur et déclenche les effets dont le `trigger` correspond. Filtre `OnSkillUsed` + `scalingSource = SkillUtilise` : l'effet ne s'exécute que si le skill porte `effet.comptageTag`. Appelé via `ApplyModuleEffect` (même chemin que les passifs de modules). Utilisé pour "votre prochain skill inflige X%", "gagnez Y armure par stack à chaque tour", etc. ⚠️ Un passif qui veut "augmenter les dégâts du prochain skill" doit utiliser `StatusBehavior.ModifyStat + DamageGainMultiplier` (lu dynamiquement) — pas `passiveEffects + ModifyStat` (qui s'empilent dans `combatStatModifiers` et ne disparaissent pas).
...
- **Tooltips (Objectif 2)** : affichage au survol — `TooltipController` (DDOL) + `TooltipTrigger` (IPointerEnter/Exit) sur chaque élément UI. Positionnement smart 4 quadrants avec padding adaptatif, badges de tags filtrés (`tag.affiché`), masquage différé (délai + fade) anti-flickering, surveillance par coroutine légère. Intégré aux skills (combat + nav), armes, modules, consommables et articles de shop.
...
- Affichage passifs des armes : pas d'affichage dédié en combat — visibles via le tooltip de l'arme
...
54. **`CloneEquipmentForLoot` — clonage des SkillData** : tout équipement obtenu (loot, event, shop, départ) passe par `CloneEquipmentForLoot()`. Après la deep copy des `skillSlots`, une boucle supplémentaire clone chaque `SkillData` via `Instantiate(slot.equippedSkill)` et applique l'héritage de tags. Ceci isole les `inheritedTags` : si deux équipements (ex : Saber + Handgun) partagent le même asset skill, leurs copies indépendantes auront chacune leur propre `inheritedTags`. **Sans ce clonage, les tags s'accumuleraient entre équipements et causeraient des bugs de ciblage/scaling.**
...
58. **`SkillIconButton.EffectiveCost`** : calculé dans `ArmSlotUI.SpawnSkillIcons()` (base + `EnergyCostModifier`), stocké sur le bouton. `UpdateSkillButtons` lit `sib.EffectiveCost` ; le remboursement à l'annulation de ciblage (`CancelTargetSelection`) lit `_pendingCoutEffectif` — jamais `skill.energyCost` directement.
...
61. **Arm3/Arm4 dans CombatManager** : `InitialiserEquipementEtSkills` itère via `Enum.GetValues(typeof(EquipmentSlot))` — couvre tous les slots présents et futurs automatiquement. Ne jamais hardcoder de liste de slots.
62. **Source équipement via callback** : `SkillIconButton` stocke `_sourceEquipment` à la création (parallèle à `availableSkills` via `_availableSkillSources`), transmis par le callback `Action<SkillData, EquipmentData>`. Ne jamais chercher l'équipement par référence `SkillData` au clic — deux équipements peuvent partager le même asset skill.
...
66. **Stats combat-temporaires vs permanentes (`ModifyStat`)** : `ArmorGainMultiplier`, `HealGainMultiplier`, `DamageGainMultiplier` et `EnergyCostReduction` sont combat-temporaires — elles ne passent jamais par `RunManager.AddStatBonus`. Dans `ApplyModuleEffect → ModifyStat`, le flag `estStatCombatTemporaire` les redirige uniquement vers `combatStatModifiers` (vidé à chaque combat → pas d'accumulation entre combats). `InitialiserStatsJoueur` les seed depuis RunManager (`ArmorGainMultiplier`, `HealGainMultiplier`, `DamageGainMultiplier`, `EnergyCostReduction`) au cas où un bonus permanent existerait (compatible futur module permanent). Les stats classiques (`Attack`, `Defense`…) restent permanentes via `RunManager.AddStatBonus` + champ `effective*`.
67. **`OnFightStart` + `ModifyStat` + scaling dynamique** : un passif d'équipement avec `trigger = OnFightStart`, `action = ModifyStat` et `scalingSource = SkillEquipeSurCetObjet` est recalculé à chaque début de combat. Le résultat va dans `combatStatModifiers` (pas RunManager). Si le trigger était `OnSkillUsed` à la place, l'effet s'empilerait à chaque skill utilisé dans le tour — à éviter pour un multiplicateur continu.
68. **`StatusDataEditor`** : custom editor masque les sections selon `behavior` (perTurnAction/effectPerStack uniquement si `PerTurnStart`, modification de stat uniquement si `ModifyStat`) et selon `decayTiming` (`decayConditionTag` uniquement si `OnSkillUse`). Tout nouveau champ `StatusData` → ajouter dans `StatusDataEditor.cs` sinon invisible.
69. **`EnergyCostReduction`** : stat combat-temporaire (positive = réduction, plancher 0). Lue via `GetCurrentEnergyCostReduction()` dans `UseSkill` (coût effectif) et `UpdateSkillButtons` (affichage). `SkillIconButton.SetDisplayedCost(int)` met à jour le texte affiché sans modifier `EffectiveCost` (coût réel).
70. **`ApplyStatusPassivesForPlayer` avant `DecayPlayerStatuses`** : ordre strict dans tous les points de déclenchement. Inverser cet ordre provoquerait l'application des passifs après que les stacks aient déjà décru — comportement incohérent pour les effets qui lisent les stacks courants.
71. **Skill cloning isolation dans `CloneEquipmentForLoot`** : après la deep copy des `skillSlots`, une boucle `foreach (var slot in equipment.skillSlots)` clone chaque SkillData avec `slot.equippedSkill = Instantiate(slot.equippedSkill)`, puis applique l'héritage de tags via `EquipSkill()`. Ceci prévient le partage accidentel de l'`inheritedTags` entre plusieurs équipements loots qui référenceraient le même asset skill. **Bug corrigé** : avant, deux weapons Saber et Handgun utilisant le même skill accumulaient les tags hérités dans une liste partagée.
72. **GameEvents order — `ApplyStatusPassivesForPlayer` puis `DecayPlayerStatuses`** : avant chaque status decay dans CombatManager, appeler d'abord `ApplyStatusPassivesForPlayer(trigger, optionalSkill)`, puis `DecayPlayerStatuses(timing, optionalSkill)`. Les passifs lisent les stacks courants ; inverser cet ordre appliquerait les effets après décroissance. Points d'appel : StartPlayerTurn, OnEndTurn, UseSkill (direct + ciblage), AppliquerDegatsAuJoueur, ApplyEffect (Heal + AddArmor).
...
77. **SkillIconButton — unique système de boutons de skill en combat** : le coût affiché est calculé dans `ArmSlotUI.SpawnSkillIcons()` (base + `EnergyCostModifier` de l'équipement source). `UpdateSkillButtons()` itère uniquement sur `_armsController.GetAllSpawnedSkillButtons()`, applique les réductions d'énergie globales (stat `EnergyCostReduction`) et met à jour `SetCooldown()`/`SetDisplayedCost()`/`SetInteractable()` en temps réel selon l'état du joueur. L'ancien `SkillButton` (boutons texte) a été supprimé — ne pas le réintroduire.
78. **TooltipTrigger sur éléments UI multiples** : chaque composant (SkillIconButton, ModuleIcon, ConsumableButton…) crée son propre TooltipTrigger dans `Setup()` avec `GetComponent()`/`AddComponent()` si absent. Ne pas créer un seul TooltipTrigger parent partagé — chaque élément possède ses propres données (nom, description, tags). Chaque `SetTooltipData()` configure l'instance locale.
79. **TooltipController.MonitorTooltipHover() coroutine légère** : s'exécute tous les 0.1s, jamais chaque frame. Résout surdetection raycast quand parent + child ont tous deux des TooltipTriggers (la coroutine vérifie souris sur élément OU sur tooltip, le raycast potentiellement bloqué par hiérarchie UI). Réduction drastique CPU — pas de ticks frame inutiles.
80. **Tooltips — `Image.raycastTarget` obligatoire** : un élément UI portant un `TooltipTrigger` doit avoir un `Graphic` (Image ou TMP) avec `raycastTarget = true`, sinon `OnPointerEnter` ne se déclenche **jamais** — échec totalement silencieux (aucune erreur, aucun log). Vérifier en particulier les `ModuleIcon` et les icônes purement décoratives.
81. **Tooltips — conflit hover parent/enfant** : les events `OnPointerEnter`/`OnPointerExit` remontent la hiérarchie UI — si un parent ET son enfant portent chacun un `TooltipTrigger`, les deux appellent `ShowTooltip()` et le dernier appelé écrase le contenu (instance `TooltipPanel` unique réutilisée). Ne placer un `TooltipTrigger` que sur l'élément le plus spécifique.
