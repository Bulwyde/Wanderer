...
69. **`EnergyCostReduction`** : stat combat-temporaire (positive = réduction, plancher 0). Lue via `GetCurrentEnergyCostReduction()` dans `UseSkill` (coût effectif) et `UpdateSkillButtons` (affichage). `SkillButton.SetDisplayedCost(int)` met à jour le texte affiché sans modifier `EffectiveCost` (coût réel).
